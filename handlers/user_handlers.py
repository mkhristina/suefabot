...
from keyboards.keyboards import select_game_kb, select_game_kb_admin
from keyboards.cube_keyboard import cube_kb, throw_or_back_kb
from lexicon.lexicon_ru import LEXICON_RU
from services.services import (
    get_bot_choice,
    get_keyboard_for_select_game,
    get_winner,
    is_admin,
)
...
from handlers.registration_handlers import is_registered
from keyboards.registration_keyboard import registration_keyboard
...
async def process_start_command(message: Message):
    ...
    if is_admin(user_id):
        await message.answer(
            "Добро пожаловать, администратор!", reply_markup=select_game_kb_admin
        )
    elif is_registered(user_id):
        await message.answer("Вы уже зарегистрированы! Добро пожаловать.")
        ...
        await message.answer(
            "Вы не зарегистрированы. Давайте начнём регистрацию.",
            reply_markup=registration_keyboard,
        )


# Этот хэндлер срабатывает на команду /admin_btn
@router.message(F.text == LEXICON_RU["admin_btn"])
async def process_admin_btn(message: Message):
    if is_admin(message.from_user.id):
        await message.answer(
            text=LEXICON_RU["admin_text_response"], reply_markup=select_game_kb_admin
        )
    else:
        await message.answer(text=LEXICON_RU["not_allow"], reply_markup=select_game_kb)
...
async def process_help_command(message: Message):
    ...
    await message.answer(
        ...
        reply_markup=get_keyboard_for_select_game(message.from_user.id),
    )


# Этот хэндлер срабатывает на согласие пользователя играть в игру камень -
#  ножницы - бумага
...
async def process_yes_answer(message: Message):
    await set_default_main_menu(message.bot, message.chat.id)
    await message.answer(
        text=LEXICON_RU["choose_game_submit"],
        reply_markup=get_keyboard_for_select_game(message.from_user.id),
    )
...
@router.message(
    F.text.in_([LEXICON_RU["rock_button"], LEXICON_RU["paper"], LEXICON_RU["scissors"]])
)
